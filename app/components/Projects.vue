...
<template>
  <div class="flex relative">
    ...
    <ul
      id="projects"
      class="mr-8 pt-32 w-full"
    >
      <li
        v-for="section in sections"
        :key="section.title"
        class="min-h-[10rem] flex gap-x-12 py-12 relative"
      >
        <h3 class="sticky top-[calc(50%-12px)] h-max w-40 whitespace-nowrap text-lg">
          {{ section.title }}
        </h3>
        <ul class="sticky top-[calc(50%-12px)] grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-14 gap-y-8 flex-1 h-max">
          <li
            v-for="repo in section.items"
            :key="repo.name"
            data-project
            class="group -mt-1.5"
          >
            <a
              :href="repo.html_url"
              target="_blank"
              class="rounded-lg group-hover:bg-neutral-100 dark:group-hover:bg-neutral-800 duration-200 transition-colors relative flex items-center px-2 py-1.5"
              :title="repo.name"
            >
              <div class="pt-1 pr-5">
                <UIcon
                  ...
                  class="size-8 opacity-50 saturate-0 group-hover:opacity-100 group-hover:saturate-100 transition-all duration-300 group-hover:delay-100"
                />
              </div>
              <div class="flex-auto">
      ...
      <li class="py-12">
        <a
          ...
          target="_blank"
          class="inline-flex items-center gap-3 rounded-lg px-4 py-3 bg-neutral-100 dark:bg-neutral-800 hover:bg-neutral-200 dark:hover:bg-neutral-700 transition-colors"
        >
          <UIcon
            name="i-carbon-pull-request"
            class="size-6"
          />
          <div>
            <p class="font-semibold">See my contributions</p>
            <p class="text-sm opacity-60">prs.onmax.me</p>
          </div>
          <UIcon
            name="i-carbon-arrow-right"
            class="size-4 ml-2 opacity-60"
          />
        </a>
      </li>
    </ul>
